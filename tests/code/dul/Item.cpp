...
{
    odil::dul::Item::Field const field(uint8_t(123));

    BOOST_REQUIRE(
        field.get_type() == odil::dul::Item::Field::Type::unsigned_int_8);
    BOOST_REQUIRE_EQUAL(field.as_unsigned_int_8(), 123);
}

BOOST_AUTO_TEST_CASE(FieldUInt16)
{
    odil::dul::Item::Field const field(uint16_t(1234));

    BOOST_REQUIRE(
        field.get_type() == odil::dul::Item::Field::Type::unsigned_int_16);
    BOOST_REQUIRE_EQUAL(field.as_unsigned_int_16(), 1234);
}

BOOST_AUTO_TEST_CASE(FieldUInt32)
{
    odil::dul::Item::Field const field(uint32_t(123456));

    BOOST_REQUIRE(
        field.get_type() == odil::dul::Item::Field::Type::unsigned_int_32);
    BOOST_REQUIRE_EQUAL(field.as_unsigned_int_32(), 123456);
}

BOOST_AUTO_TEST_CASE(FieldString)
{
    odil::dul::Item::Field const field("abcdef");

    BOOST_REQUIRE(field.get_type() == odil::dul::Item::Field::Type::string);
    BOOST_REQUIRE_EQUAL(field.as_string(), "abcdef");
}

BOOST_AUTO_TEST_CASE(FieldItems)
{
    odil::dul::Item const item_1({{"foo", uint8_t(123)}});
    odil::dul::Item const item_2({{"bar", std::string("abcdef")}});
    odil::dul::Item::Field const field(
        std::vector<odil::dul::Item>({item_1, item_2}));

    BOOST_REQUIRE(field.get_type() == odil::dul::Item::Field::Type::items);
    BOOST_REQUIRE_EQUAL(field.as_items().size(), 2);
}

BOOST_AUTO_TEST_CASE(DefaultConstructor)
{
    odil::dul::Item const item;
...
{
    odil::dul::Item const sub_item(
        {{"bar", std::string("abcdef")}});
    odil::dul::Item item;
    item.add("foo", std::vector<odil::dul::Item>({sub_item}));
    BOOST_REQUIRE_EQUAL(item.size(), 1);
