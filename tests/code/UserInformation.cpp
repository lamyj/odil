...
{
    dcmtkpp::UserInformation const item;
...
BOOST_AUTO_TEST_CASE(ConstructorStreamRQ)
{
    ...
    dcmtkpp::UserInformation item(stream);
...
{
    dcmtkpp::UserInformation item;

    BOOST_REQUIRE(!item.has_maximum_length());
    BOOST_REQUIRE_THROW(item.get_maximum_length(), dcmtkpp::Exception);

    item.set_maximum_length(dcmtkpp::MaximumLength(0x12345678));
...
BOOST_AUTO_TEST_CASE(UserIdentityBoth1)
{
    ...
    dcmtkpp::UserIdentityRQ user_identity_rq;
    ...
    dcmtkpp::UserIdentityAC user_identity_ac;
